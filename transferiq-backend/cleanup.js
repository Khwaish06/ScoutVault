...
async function cleanupDuplicates() {
  try {
    ...
    for (const abbrevPlayer of abbreviatedPlayers) {
      ...
      if (nameParts.length >= 2) {
        ...
        if (potentialMatches.length > 0) {
          ...
          console.log(`🎯 ${DRY_RUN ? 'Would remove' : 'Removing'} abbreviated: "${abbrevPlayer.name}" in favor of "${fullNamePlayer.name}"`);
    ...
    for (const [team, players] of Object.entries(playersByTeam)) {
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        if (duplicates.length > 0) {
          ...
          console.log(`   ${DRY_RUN ? 'Would keep' : 'Keeping'}: "${bestVersion.player.name}"`);
    ...
    console.log(`\n📊 Cleanup Summary:`);
    console.log(`   Players ${DRY_RUN ? 'would be' : ''} removed: ${totalRemoved}`);
    console.log(`   Players ${DRY_RUN ? 'would be' : ''} updated: ${totalUpdated}`);
    console.log(`   ${DRY_RUN ? 'Current' : 'Final'} player count: ${finalCount.toLocaleString()}`);
    console.log(`   Estimated duplicates ${DRY_RUN ? 'that would be' : ''} cleaned: ${totalRemoved}`);

    if (DRY_RUN) {
      console.log(`\n✅ Dry run complete. No changes were made to the database.`);
      console.log(`   To run the actual cleanup: node cleanup.js`);
    } else {
